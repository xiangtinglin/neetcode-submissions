...
/*
2️⃣ 🏅Stack- T:O(n); S:O(n)
    # 創建2容器(stack & ans), 只走一次。 -> stack用來逐一暫存待處理的溫度& index, 找到更大則pop out!
*/
...
public:
    vector<int> dailyTemperatures(vector<int>& temperatures) {
        int n = static_cast<int>(temperatures.size());
        vector<int> ans(n, 0); // #初始化
        stack<pair<int, int>> stack;

        for (int i = 0; i < n; i++) {
            int t = temperatures[i];
            while (!stack.empty() && t > stack.top().second) {
                auto temPair = stack.top();
                stack.pop();
                ans[temPair.first] = i - temPair.first;
            }
            stack.push({i, t}); // 注意: pair -> 花括弧!!!         
        }
        return ans;
    }
};
